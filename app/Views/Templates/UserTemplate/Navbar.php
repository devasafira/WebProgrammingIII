<div class="overflow-x-hidden relative mb-[4rem]">
    <!-- container nav -->
    <div class="w-[100vw] h-[9vh] p-[1rem] px-[1.5rem] fixed top-0 left-0 right-0 bg-white z-30">
        <!-- navbar -->
        <nav class="flex justify-between items-end poppins">
            <!-- logo and title -->
            <div class="flex font-[600]">
                <h1 class="text-[1.5rem]">Sushi <span class="text-[#df5518]">Go</span></h1>
                <p class="mx-[0.3rem] mt-[-0.1rem] text-[1.7rem] text-[#df5518]"><i class='bx bx-sushi'></i></p>
            </div>
            <!-- navbar links -->
            <div class="navbar">
                <ul class="flex text-[1.15rem] text-[500]">
                    <li class="mx-[1.2rem] transition ease-in duration-300 hover:text-[#df5518]"><a href="">Home</a></li>
                    <li class="mx-[1.2rem] transition ease-in duration-300 hover:text-[#df5518]"><a href="">Menu</a></li>
                    <li class="mx-[1.2rem] transition ease-in duration-300 hover:text-[#df5518]"><a href="">Contact Us</a></li>
                    <li class="mx-[1.2rem] transition ease-in duration-300 hover:text-[#df5518]"><a href=""><i class='bx bxs-cart'></i></a></li>
                </ul>
            </div>
        </nav>
    </div>
</div>
</html>